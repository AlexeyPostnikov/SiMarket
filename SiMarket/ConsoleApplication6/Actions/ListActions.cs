...
namespace SiMarket
{
    class ListActions
    {
        ...
        public void SortUp()
        {
            List.Sort(delegate (SharePacket SP1, SharePacket SP2)
            { return SP1.Price.CompareTo(SP2.Price); });
        ...
        public List<SharePacket> FindType(string companyType)
        {
            int i = 0;
            List<SharePacket> packets = new List<SharePacket>();
            foreach (SharePacket packet in Market.Current.CompaniesList.List)
            {
                if (Market.Current.CompaniesList.List[i].CompanyType == companyType)
                {
                    packets.Add(packet);
                }
                i++;
